    ...
    public void requestFreeLicense(FreeLicenseRequest request) {
        ...
        try {
            ...
            if (response.isSuccessful() && response.body() != null) {
                LOG.debug("Received free tbSyslog Enterprise license: {}", response.body());
                ...
                if (response.errorBody() != null) {
                    LOG.error("Couldn't request free tbSyslog Enterprise license: {} (code={})", response.errorBody().string(), response.code());
                } else {
                    LOG.error("Couldn't request free tbSyslog Enterprise license: {} (code={}, message=\"{}\")", response.message(), response.code(), response.message());
                }
                throw new FreeLicenseRequestException("Couldn't request free tbSyslog Enterprise license", request);
            }
        } catch (FreeLicenseRequestException e) {
            // no need to log this again
            throw e;
        } catch (IOException e) {
            LOG.error("Couldn't request free tbSyslog Enterprise license from remote service", e);
            throw new FreeLicenseRequestException("Couldn't request free tbSyslog Enterprise license from remote service", request, e);
        } catch (Exception e) {
            LOG.error("Couldn't request free tbSyslog Enterprise license", e);
            throw new FreeLicenseRequestException("Couldn't request free tbSyslog Enterprise license", request, e);
        }
    }
}
